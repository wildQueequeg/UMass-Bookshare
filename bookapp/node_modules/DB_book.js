...
exports.searchBook = function searchBook(query_, function_)
{
	...
	bookresult.then(function(record){
		// search.searchHandler(record, function_); //TODO change up searching
		var resultsAr = [];
		console.log(record);
		if(record != null){
			...
			{
				resultsAr.push(
						{title : record[0][i].title.trim(),
						 author : record[0][i].author.trim(),
						 isbn10 : record[0][i].isbn10,
						 isbn13 : record[0][i].isbn13,
						 publicationdate : record[0][i].publicationdate,
